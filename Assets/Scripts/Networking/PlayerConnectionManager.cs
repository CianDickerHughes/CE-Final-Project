...
public class PlayerConnectionManager : NetworkBehaviour
{
    ...
    private void NotifyKickedClientRpc(ulong targetClientId)
    {
        if (NetworkManager.Singleton.LocalClientId == targetClientId)
        {
            ...
            // Set the kick flag so the Campaigns scene shows a notification
            KickNotification.SetKicked("You have been kicked from the session.");

            // Shut down the network on this client and go back to Campaigns
